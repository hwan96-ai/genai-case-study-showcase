...
- **공개명 정리:** 2026-06-28부터 공개 제목·카드·링크·파일명은 설명형 이름만 사용합니다. 내부 추적 ID는 PUBLIC 산출물에 표시하지 않으며, 세부 매핑은 비공개 SSOT에서만 관리합니다.
...
고객사·콜봇 플랫폼·월패드 제조사 등 미승인 실명은 포함하지 않습니다. 승인 목록은 공개 저장소 밖의 비공개 승인 기록에 두며, PUBLIC 저장소에서 내부 경로나 파일명을 직접 참조하지 않습니다. validator가 현재 승인 정보를 count/boolean으로 확인하지 못하면 익명 표기를 유지합니다.
